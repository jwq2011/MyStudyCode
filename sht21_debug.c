...
#define SENSOR_GET_SHT21_TEMPERATURE	0x80000001
#define SENSOR_GET_SHT21_HUMIDITY		0x80000002

#define SHTX_MAJOR		0

static struct cdev cdev;
static struct class	*sht21_class;
static struct sht21 *sht21_data = NULL;
static int sht21_major = SHTX_MAJOR;
 ...
 */
#define TEMP_LEN		5
unsigned char mem[TEMP_LEN];

static void int_to_bytes(int data, unsigned char buf[], int start)
{
	unsigned char i;
	for(i = 0; i < 5; i++)
	{
		buf[i] = data >> (8*(DATA_LENGTH-1-i));
	}
}
 
static int sht21_cdev_open(struct inode *inode,struct file *filp)
{
	return 0;
}
 
static ssize_t sht21_cdev_read(struct file *filp,char __user *buf, size_t count, loff_t *ppos)
{
	
	unsigned char j, ;
	int data[2];

	data[0] = sht21_data->temperature;
	data[1] = sht21_data->humidity;

	int_to_bytes(data[0], mem, 0);

	if(copy_to_user(buf, (void *) temp, DATA_LENGTH))
	{
		return -EINVAL;
	}
	return count;
...
static ssize_t sht21_cdev_write(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{return 0;}


static long sht21_cdev_ioctl(struct file * filep, unsigned int cmd, unsigned long arg)
{
	static int temp_val, err;

	err = sht21_update_measurements(sht21_data->client);
	if (err < 0)
		return err;

	switch(cmd)
	{
		case SENSOR_GET_SHT21_TEMPERATURE:
			temp_val = sht21_data->temperature;
			printk("temperature = %d\n", temp_val);
			break;
		case SENSOR_GET_SHT21_HUMIDITY:
			temp_val = sht21_data->humidity;
			printk("\nhumidity = %d\n", temp_val);
			break;
		default:
			return -EINVAL;
	...
	if( copy_to_user((int *)arg, &temp_val, 1) )
	{
		return -EFAULT;
	}
	return 0;
...
static int sht21_cdev_close(struct inode *inode,struct file *filp)
{return 0;}


static struct file_operations sht21_cdev_fops = {
	.owner			= THIS_MODULE,
	.open			= sht21_cdev_open,
	.read			= sht21_cdev_read,
	.write			= sht21_cdev_write,
	.unlocked_ioctl	= sht21_cdev_ioctl,
	.release		= sht21_cdev_close,
};

static int sht21_cdev_init(void)
{
	int result, err;

	dev_t devno = MKDEV(sht21_major, 0);
	if(sht21_major)
	{
		result = register_chrdev_region(devno, 1, "sht21_ch");
	}
	else
	{
		result = alloc_chrdev_region(&devno, 0, 1, "sht21_ch");
		sht21_major = MAJOR(devno);
	}

	if(result < 0)
	{
		printk (KERN_WARNING "hello: can't get major number %d/n", sht21_major);
		return result;
	}

	cdev_init(&cdev, &sht21_cdev_fops);
	cdev.owner = THIS_MODULE;
	err = cdev_add(&cdev, devno, 1);

	sht21_class = class_create(THIS_MODULE, "sht21_class");
	if(IS_ERR(sht21_class))
	{
		printk("Err: failed in creating class./n");
		return -1; 
	}
	device_create(sht21_class, NULL, devno, NULL, "sht21_debug");
	printk("==========%s: success\n", __FUNCTION__);
	...
	const struct i2c_device_id *id)
{
	...
	memset(sht21, 0, sizeof(struct sht21));
...
{
	i2c_add_driver(&sht21_driver);
	return sht21_cdev_init();
}
